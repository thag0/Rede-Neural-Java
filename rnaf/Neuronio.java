...
public class Neuronio implements Serializable{
    ...
    */
   public Neuronio(int ligacoes, float alcancePeso, int inicializador){
      if (alcancePeso <= 0) throw new IllegalArgumentException("O valor de alcance do peso deve ser positivo e diferente de zero.");
   
      this.entradas = new float[ligacoes];
      this.momentum = new float[ligacoes];
      this.acumuladorGradiente = new float[ligacoes];
      this.acumuladorSegundaOrdem = new float[ligacoes];
      this.gradiente = new float[ligacoes];
      this.gradienteAcumulado = new float[ligacoes];

      this.pesos = new float[ligacoes];
    ...
    */
   private void inicializacaoAleatoria(float alcancePeso){
      for(int i = 0; i < pesos.length; i++){
         this.pesos[i] = random.nextFloat(-alcancePeso, alcancePeso);
      }
   }
   ...
   private void inicializacaoHe(int entradas){
      float desvioPadrao = (float) (Math.sqrt(2.0 / entradas));
      for(int i = 0; i < pesos.length; i++){
         this.pesos[i] = (float) (random.nextGaussian() * desvioPadrao);
      }
   }
   ...
   private void inicializacaoLeCun(int entradas){
      float desvioPadrao = (float) Math.sqrt(1.0 / entradas);
      for(int i = 0; i < pesos.length; i++){
         this.pesos[i] = (float) (random.nextGaussian() * desvioPadrao);
      }
   }
